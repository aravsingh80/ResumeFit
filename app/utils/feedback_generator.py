...
def get_gen_pipeline():
    ...
    if _gen_pipeline is None:
        ...
        _gen_pipeline = pipeline("text-generation", model="distilgpt2")
...
def generate_feedback(resume_text, job_description, sim_threshold=0.65, max_segs=3):
    ...
    for segment in segments:
        ...
        if sim < sim_threshold:
            prompt = f'The candidate resume does not mention "{segment}".'
...
def analyze_resume_with_feedback(resume_text, job_description, model_score, cosine_weight=0.1):
    ...
    # Always generate feedback (forced)
    suggestions = generate_feedback(resume_text, job_description, max_segs=5)
